...
When a change affects a document's contents, note it rather than rewriting the document immediately. Only update documentation at the end of a work session or when explicitly asked. Update an entry below only when a document's scope or responsibilities change.
